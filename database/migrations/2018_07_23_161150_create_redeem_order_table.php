...
class CreateRedeemOrderTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('redeem_order', function (Blueprint $table) {
            ...
            $table->string('redeem_currency', 10)->default('')->comment('订单货币');
            $table->string('redeem_amount', 10)->default('')->comment('订单金额');
            $table->string('token_name', 10)->default('')->comment('token名称');
            $table->string('token_amount', 10)->default('')->comment('token数量');
            $table->string('bank_account', 100)->default('')->comment('赎回银行账户');
            $table->string('bank_account_bsb', 10)->default('')->comment('赎回银行BSB');
            $table->string('bank_account_nmae', 10)->default('')->comment('赎回银行账户姓名');
